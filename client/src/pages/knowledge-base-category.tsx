...
import {
  Search, BookOpen, Clock, Eye, ArrowRight, Tag, ChevronRight, ArrowLeft, Layers, BookMarked
} from "lucide-react";
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              {category.description && (
                <p className="text-gray-500 mt-1 max-w-2xl">{category.description}</p>
              )}
              <p className="text-sm text-gray-400 mt-2">{articles.length} article{articles.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-4 sticky top-24">
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={`w-full text-left text-sm px-3 py-1.5 rounded-lg transition-colors ${!activeTag ? "bg-amber-100 text-amber-700 font-medium" : "text-gray-600 hover:bg-gray-100"}`}
                  >
                    All articles
                  </button>
                  {allTags.map(tag => (
                    <button
                      ...
                      className={`w-full text-left text-sm px-3 py-1.5 rounded-lg transition-colors ${activeTag === tag.slug ? "bg-amber-100 text-amber-700 font-medium" : "text-gray-600 hover:bg-gray-100"}`}
                    >
                      {tag.name}
                    </button>
                  ))}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <Select value={sort} onValueChange={setSort}>
                <SelectTrigger className="w-40 h-9 text-sm rounded-xl border-gray-200">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {articles.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
                ...
                <p className="text-gray-500">No articles found{activeTag ? " for this tag" : ""}.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {articles.map(article => (
                  <Link key={article.id} href={`/knowledge-base/article/${article.slug}`}>
                    <div className="group flex items-start gap-4 p-5 bg-white rounded-2xl border border-gray-200/80 hover:border-amber-200 hover:shadow-md hover:shadow-amber-500/5 transition-all duration-200 cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 group-hover:text-amber-700 transition-colors line-clamp-1 mb-1">
                          {article.title}
                        </h3>
                        {article.summary && (
                      ...
                      <div className="flex-shrink-0 flex flex-col items-end gap-1 text-xs text-gray-400">
                        ...
                        <ArrowRight className="w-4 h-4 text-amber-400 mt-1 group-hover:translate-x-0.5 transition-transform" />
                      </div>
                    </div>
                  </Link>
                ))}
