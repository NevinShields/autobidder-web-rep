...
export default function UpsellForm() {
  ...
  // Calculate totals including upsells
  ...
  const upsellTotal = selectedUpsells.reduce((sum, upsellId) => {
    ...
    return sum + (upsell ? Math.round(subtotal * (upsell.percentageOfMain / 100)) : 0);
  }, 0);
  const bundleDiscount = styling.showBundleDiscount && selectedServices.length > 1 
    ...
    : 0;
  const discountedSubtotal = subtotal - bundleDiscount + upsellTotal;
  ...
  const handleContinueToUpsells = () => {
    if (selectedServices.length === 0) {
      toast({
        ...
        description: "Choose the services you need before proceeding.",
        variant: "destructive",
      });
      return;
    }
    
    if (!areAllServiceVariablesComplete()) {
      toast({
        title: "Complete service configuration",
  ...
  const handleSubmitQuoteRequest = () => {
    ...
    const leadData = {
      ...
      upsells: selectedUpsells.map(upsellId => {
        const upsell = upsellServices.find(u => u.id === upsellId);
        const upsellPrice = Math.round(subtotal * ((upsell?.percentageOfMain || 0) / 100));
        return {
          id: upsellId,
          name: upsell?.name || "Unknown Upsell",
          description: upsell?.description || "",
          category: upsell?.category || "addon",
          percentageOfMain: upsell?.percentageOfMain || 0,
          calculatedPrice: upsellPrice
        };
      }),
      baseTotal: subtotal,
