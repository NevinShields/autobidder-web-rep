const performList = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/triggers/estimate-accepted`,
    headers: {
...
const performSubscribe = async (z, bundle) => {
  ...
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/hooks/estimate-accepted/subscribe`,
    method: 'POST',
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
...
const getSample = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/sample/estimate-accepted`,
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
    },
  });
...
module.exports = {
  ...
  display: {
    ...
    description: 'Triggers when a customer accepts an estimate.',
  },
  operation: {
    type: 'hook',
