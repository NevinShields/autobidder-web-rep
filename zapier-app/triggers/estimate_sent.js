const performList = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/triggers/estimate-sent`,
    headers: {
...
const performSubscribe = async (z, bundle) => {
  ...
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/hooks/estimate-sent/subscribe`,
    method: 'POST',
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
...
const getSample = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/sample/estimate-sent`,
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
    },
  });
...
module.exports = {
  ...
  display: {
    label: 'Estimate Sent',
    description: 'Triggers when an estimate is sent to a customer via email or SMS.',
  },
  operation: {
    type: 'hook',
