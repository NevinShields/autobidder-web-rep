const performList = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/triggers/estimate-viewed`,
    headers: {
...
const performSubscribe = async (z, bundle) => {
  ...
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/hooks/estimate-viewed/subscribe`,
    method: 'POST',
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
...
const getSample = async (z, bundle) => {
  const response = await z.request({
    url: `${bundle.authData.server_url}/api/zapier/sample/estimate-viewed`,
    headers: {
      Authorization: `Bearer ${bundle.authData.api_key}`,
    },
  });
...
module.exports = {
  ...
  display: {
    ...
    description: 'Triggers when a customer views their estimate.',
  },
  operation: {
    type: 'hook',
